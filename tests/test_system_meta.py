...
import pytest
from surfmeta.utils import get_system_info, calculate_local_checksum, calculate_remote_checksum
from surfmeta.system_metadata import local_meta, snellius_meta, meta_checksum
...
def test_get_system_info(monkeypatch):
    ...
    assert get_system_info() == "test-node"
...
def test_calculate_local_checksum(tmp_path: Path):
    ...
    result = calculate_local_checksum(test_file, "md5")
...
def test_calculate_local_checksum_invalid_algorithm(tmp_path: Path):
    ...
    with pytest.raises(ValueError):
        calculate_local_checksum(test_file, "unsupportedalgo")
...
def test_calculate_remote_checksum(mock_run):
    ...
    checksum = calculate_remote_checksum("host", "user", Path("/remote/file"), "md5")
...
def test_calculate_remote_checksum_invalid_algorithm():
    ...
    with pytest.raises(ValueError):
        calculate_remote_checksum("host", "user", Path("/file"), "invalidalgo")


@patch("surfmeta.system_metadata.calculate_remote_checksum", return_value="fakechecksum")
def test_meta_checksum_remote(mock_remote):
    ...
    updated = meta_checksum(meta, file_path, remote=True, host="host", username="user", algorithm="md5")
...
def test_meta_checksum_remote_missing_params():
    ...
    with pytest.raises(ValueError):
        meta_checksum({}, Path("/file.txt"), remote=True, host=None, username=None)
...
def test_meta_checksum_local_file(tmp_path: Path):
    ...
    meta = {}
    result = meta_checksum(meta, f, algorithm="md5")
...
def test_meta_checksum_local_missing_file(tmp_path: Path):
    ...
    with warnings.catch_warnings(record=True) as w:
        result = meta_checksum(meta, missing)
        assert len(w) == 1
